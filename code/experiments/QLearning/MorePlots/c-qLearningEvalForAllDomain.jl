...
for (i,lr_setting) in enumerate(lr_settings)  
    ...
    for (j,pair) in enumerate(VI_ret) 
        domain, results = pair
        ax = Axis(f[j, i], title="κ=$(lr_setting[37:end-5]) Q-learning ($domain)") 
...
sideinfo2 = Label(f[8, 1:4], "Quantile level", textsize = 30)
...
l = Legend(f[8, 1:3],obj_scatter,["q̃ᵈ ","q̲ᵈ ","π̃ performance","π̲ performance"],orientation = :horizontal)
